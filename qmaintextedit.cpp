...
bool QMainTextEdit::eventFilter(QObject *obj, QEvent *event)
{
    if(obj == this){
...
void QMainTextEdit::paintEvent(QPaintEvent* event)
{
    if (length > 0){
...
void QMainTextEdit::setLengthChecker(TransDocument *lc)
{
    this->lengthChecker = lc;
}

void QMainTextEdit::setSize(TransDocument::rectSize sz)
{
    this->size = sz;
}

void QMainTextEdit::checkLength()
{
    QList<float> lengthList = lengthChecker->lengthValidator(this->toPlainText(),this->size);
    length = lengthList.at(2);
}

void QMainTextEdit::hideTooltip()
{
    QToolTip::hideText();
}

void QMainTextEdit::keyPressEvent(QKeyEvent *evt)
{
...
/*
void QMainTextEdit::contextMenuEvent(QContextMenuEvent *event)
{

}
*/

void QMainTextEdit::setChambre(ChambrePanel * ch)
{
