...
QMainAllergiesTextEdit::QMainAllergiesTextEdit(QWidget *parent) :
    QTextEdit(parent)
{
...
bool QMainAllergiesTextEdit::eventFilter(QObject *obj, QEvent *event)
{
    if(obj == this){
...
void QMainAllergiesTextEdit::paintEvent(QPaintEvent* event)
{
    if (length > 0 &&(this->hasFocus || antecedents->focus() || allergiesCheck->hasFocus()) && allergiesCheck->isChecked()){
...
void QMainAllergiesTextEdit::setLengthChecker(TransDocument *lc)
{
    this->lengthChecker = lc;
}

void QMainAllergiesTextEdit::setSize(TransDocument::rectSize sz)
{
    this->size = sz;
}

void QMainAllergiesTextEdit::setPartner(QMainAtcdTextEdit *atcd)
{
    antecedents = atcd;
}

void QMainAllergiesTextEdit::setAllergiesCheck(QCheckBox *allergiesChk)
{
    allergiesCheck = allergiesChk;
}

void QMainAllergiesTextEdit::setLength(float l)
{
    length = l;
}

bool QMainAllergiesTextEdit::focus()
{
    return this->hasFocus;
}

void QMainAllergiesTextEdit::checkLength()
{
    QList<float> lengthList = lengthChecker->lengthValidator(this->toPlainText(),this->size);
    length = lengthList.at(2);
}

void QMainAllergiesTextEdit::hideTooltip()
{
    QToolTip::hideText();
}

void QMainAllergiesTextEdit::keyPressEvent(QKeyEvent *evt)
{
    QTextEdit::keyPressEvent(evt);
    QList<float> lengthList = lengthChecker->lengthValidator(TexteMisEnForme(),this->size);
    length = lengthList.at(2);
    antecedents->setLength(length);
    antecedents->viewport()->repaint(0,0,3,antecedents->height());
...
QString QMainAllergiesTextEdit::TexteMisEnForme()
{
    return antecedents->toPlainText()+" allergies : "+this->toPlainText()+"";

}

void QMainAllergiesTextEdit::setChambre(ChambrePanel * ch)
{
